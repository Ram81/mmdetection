...
#     'appliances',
#     'clothes',)
# classes = json.loads(open("data/hm3d_semantic/hm3d_objects_1scene_train.json").read())

data = dict(
    samples_per_gpu=12,  # Batch size of a single GPU
    workers_per_gpu=8,  # Worker to pre-fetch data for each single GPU
    train=dict(
        img_prefix='data/hm3d_semantic/segmentation_coco_270/train/',
        classes=classes,
        ann_file='data/hm3d_semantic/segmentation_coco_270/annotations/hm3d_train.json'),
    val=dict(
        img_prefix='data/hm3d_semantic/segmentation_coco_270/train/',
        classes=classes,
        ann_file='data/hm3d_semantic/segmentation_coco_270/annotations/hm3d_train.json'),
